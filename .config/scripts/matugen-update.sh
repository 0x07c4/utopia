...
WALL_HASH=$(md5sum "$WALLPAPER" | awk '{print $1}')
...
# 执行前兜底
if ! matugen image "$TARGET_IMAGE" --dry-run &>/dev/null; then
	if [ "$TARGET_IMAGE" != "$CACHED_IMAGE" ]; then
		[ -f "$CACHED_IMAGE" ] || convert_to_cache
		[ -f "$CACHED_IMAGE" ] && TARGET_IMAGE="$CACHED_IMAGE"
	fi
fi
