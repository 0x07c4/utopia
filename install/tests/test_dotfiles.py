...
class DotfilesTest(unittest.TestCase):
    ...
    def test_preflight_rejects_uninitialized_nvim_submodule(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (target / "etc/passwd").write_text(
                f"utopia:x:{os.getuid()}:{os.getgid()}::/home/utopia:/bin/zsh\n"
            )
    ...
    def test_execution_assigns_generated_parent_directories_to_target_user(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            source = Path(temporary) / "source"
            home = Path(temporary) / "target/home/utopia"
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                [[
                    ...
                    "rime_deployer",
                    "--build",
                    "/home/utopia/.local/share/fcitx5/rime",
                    "/usr/share/rime-data",
                    "/home/utopia/.local/share/fcitx5/rime/build",
                ]],
            )
